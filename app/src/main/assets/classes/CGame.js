function CGame(oData) {
    ...
    this.startFade = function() {
        ...
        createjs.Tween.get(_oFade).to({
            alpha: 0
        }, MS_FADE_ANIM, createjs.Ease.cubicOut).call(function() {
    ...
    this.createStartStacks = function() {
        ...
        _oPrevStack = new CStack({
            x: CANVAS_WIDTH_HALF,
            y: CANVAS_HEIGHT_HALF + 150,
            dir: NO_DIR
        }, {
            line: COLOR_GRADIENTS_STACKS[_iColorPos],
            plane: COLOR_GRADIENTS_STACKS[_iColorPos],
            width: LINES_WIDTH,
            depth: null
        ...
        var oInfo = {
            x: STACKS_START_POSITION[_iStartPos].x,
            y: STACKS_START_POSITION[_iStartPos].y - STACK_DEPTH_SIZE,
            dir: STACKS_START_POSITION[_iStartPos].dir
        };

        _oCurrentStack = new CStack(oInfo, {
            line: COLOR_GRADIENTS_STACKS[_iColorPos],
            plane: COLOR_GRADIENTS_STACKS[_iColorPos],
            width: LINES_WIDTH,
            depth: null
    ...
    this.zoomOutContainerStacks = function() {
        ...
        createjs.Tween.get(_oContainerStack).to({
            scaleX: fZoom,
            scaleY: fZoom,
            y: fPosY
        }, MS_TIME_ZOOM_OUT, createjs.Ease.quartOut);
    };
    ...
    this.intersectionPointsStacks = function(oPrevStack, oCurrentStack) {
        ...
        if (iSidesEdges === 0) {
            if (iCutDimRightPointAX !== 0 && iCutDimLeftPointBX !== 0) { //RIGHT
                oPointOffset = {
                    x: iCutDimRightPointAX,
                    y: iCutDimRightPointAY
                };
                if (Math.abs(iCutDimRightPointAX) > PERFECT_RANGE && Math.abs(iCutDimLeftPointBX) > PERFECT_RANGE) { //RIGHT
                    ...
                    this.newEdgesPosition({
                        right: aCurrentStackEdges[0],
                        left: aCurrentStackEdges[1],
                        distance: aCurrentStackSpecularEdges[1],
                        point1: {
                            x: iCutDimRightPointAX,
                            y: iCutDimRightPointAY
                        },
                        point2: {
                            x: iCutDimLeftPointBX,
                            y: iCutDimLeftPointBY
                        }
                    });
                    bCut = true;
                }
                ...
                oPointOffset = {
                    x: iCutDimLeftPointAX,
                    y: iCutDimLeftPointAY
                };
                if (Math.abs(iCutDimRightPointBX) > PERFECT_RANGE && Math.abs(iCutDimLeftPointAX) > PERFECT_RANGE) {
                    ...
                    this.newEdgesPosition({
                        right: aCurrentStackEdges[1],
                        left: aCurrentStackEdges[0],
                        distance: aCurrentStackSpecularEdges[0],
                        point1: {
                            x: iCutDimRightPointBX,
                            y: iCutDimRightPointBY
                        },
                        point2: {
                            x: iCutDimLeftPointAX,
                            y: iCutDimLeftPointAY
    ...
    this.cutStack = function(oStartEdge, oPointsCut, oPointOffset, bRightCol) {
        ...
        var oInfo = {
            x: _oCurrentStack.getPos().x,
            y: _oCurrentStack.getPos().y,
            dir: NO_DIR
        };
        if (SHOW_CUT_EDGES) {
        ...
        var oCutStack = this.createStack(aEdges, {
            x: 0,
            y: 0
        }, oInfo, _oCurrentStack.getColor());
    ...
    this.createNewStack = function(aEdges, oOffset, bCut) {
        ...
        if (!bCut) {
            ...
            _oCurrentStack.updateEdgesPosition();
            oInfo = {
                x: _oCurrentStack.getPos().x + _oOffset.x - oOffset.x,
                y: _oCurrentStack.getPos().y - STACK_DEPTH_SIZE + _oOffset.y - oOffset.y,
                dir: STACKS_START_POSITION[_iStartPos].dir
            };
        } else {
            playSound("cut", 1, false);
            oInfo = {
                x: _oCurrentStack.getPos().x - oOffset.x,
                y: _oCurrentStack.getPos().y - STACK_DEPTH_SIZE - oOffset.y,
                dir: STACKS_START_POSITION[_iStartPos].dir
            };
        }
        ...
        var oCurrentStack = this.createStack(aEdges, oOffset, oInfo, {
            line: COLOR_GRADIENTS_STACKS[_iColorPos],
            plane: COLOR_GRADIENTS_STACKS[_iColorPos],
            width: LINES_WIDTH,
            depth: null
    ...
    this.moveContainerStack = function() {
        ...
        createjs.Tween.get(_oContainerStack, {
            override: true
        }).to({
            y: _iContainerY
        }, MS_SCROLL_CONTAINER_STACK, createjs.Ease.cubicOut).call(function() {
    ...
    this.getLimitPos = function(oPos) {
        ...
        for (var i = 0; i < STACKS_LIMIT_POSITION.length; i++) {
            aLimitPosition.push({
                x: oPos.x + (DIRECTION_STACKS[i].x * STACKS_DISTANCE_OFFSET),
                y: oPos.y + (DIRECTION_STACKS[i].y * STACKS_DISTANCE_OFFSET)
            });
        }
