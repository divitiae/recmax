...
function sizeHandler() {
    ...
    s_iScaleFactor = 1;Math.min((h / CANVAS_HEIGHT), (w / CANVAS_WIDTH));

    var destW = CANVAS_WIDTH * (w / CANVAS_WIDTH);
    var destH = CANVAS_HEIGHT * (h / CANVAS_HEIGHT);
    ...
    if (fOffsetX * fGameInverseScaling < -EDGEBOARD_X ||
        ...
        destW = CANVAS_WIDTH * (w / (CANVAS_WIDTH - (EDGEBOARD_X * 2)));
        destH = CANVAS_HEIGHT * (h / (CANVAS_HEIGHT - (EDGEBOARD_Y * 2)));
        fOffsetY = (h - destH) / 2;
        fOffsetX = (w - destW) / 2;
    ...
    if (s_bIsIphone) {
        ...
        s_oStage.scaleX = (destW / CANVAS_WIDTH) *2;
		s_oStage.scaleY = (destH / CANVAS_HEIGHT) * 2;
        ...
        s_oStage.scaleX = destW / CANVAS_WIDTH;
		s_oStage.scaleY = destH / CANVAS_HEIGHT;//s_iScaleFactor;
    }
...
function colorLerpShape(oShape, oStart, oEnd, iWait, iDuration, oShapeFunc, oEase, aEdges, szProperties, oCallFunc) {
    var oLerp = oStart;
    var oColor = {};
    var oTween = createjs.Tween.get(oLerp, {
        override: true
    }).wait(iWait).to({
        lerp: oEnd.lerp
    }, iDuration, oEase).call(function() {
        if (oCallFunc !== null) {
            oCallFunc();
        }
